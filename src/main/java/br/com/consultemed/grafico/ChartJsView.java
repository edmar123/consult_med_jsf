package br.com.consultemed.grafico;
...
public class ChartJsView implements Serializable { 
	 private PieChartModel pieModel;
	    ...
	    public void createBarModel() {
	        ...
	        barDataSet.setLabel("Consultas agendadas");
	        ...
	        for (Agendamento agendamento : agendamentos) {
				...
				}else if (agendamento.getStatus().equals(StatusAgendamento.CANCELADO)) {
					totalCancelado = totalCancelado + 1;
				}else {
					totalReagendado = totalReagendado + 1;
				}
			}
	          
	        List<Number> values = new ArrayList<>();

	        values.add(totalAgendado);
	        values.add(0);
	        values.add(0);
	        values.add(totalCancelado);
	        values.add(0);
	        values.add(0);
	        values.add(totalReagendado);
